...
function generateNextIds(existing) {
  ...
  const hotIds = candidates.filter(id => {
    ...
    return existing.some(r => r.id.slice(4,6) === venue && r.id.slice(6,8) === kai);
  });
...
async function main() {
  ...
  for (const id of targetIds) {
    ...
    if (result) {
      ...
      console.log(`✓ ${result.name||id} ${result.horses.length}頭 [${result.track}${result.distance}m ${result.cond}] オッズ:${s?.odds} 人気:${s?.popular}`);
    } else {
      console.log(`- なし: ${id}`);
    }
    await sleep(5000);
